...
int main(int argc,char* argv[]){
    if(argc == 1){
        ...
        char prompt[128] = "prompt> "; //char prompt to prompt the user
        ...
        { // Interactive Mode loop
            ...
            //reads the line from a command line argument
            {
                ...
                for(j=0;j<i;j++)
                {
                    ...
                    {
                        newPrompt = strtok(input," ");
                        // newPrompt = "prompt"
                        newPrompt = strtok(NULL,"");
                        // newPrompt = "desired string"
                    ...
                    while(fgets(history_buff,128,(FILE*)history)
                    != '\0'){ // Doesnt work right now
                    printf("History: %s\n",history_buff);
                    }
                    }
                    */ 
    ...
    else if (argc==2)
    {
        ...
        printf("****Batch Mode****\n");
        int i = 0;
        // i and j declared to used in for loop
        int j = 0;
        int exit_flag = FALSE;
        //boolean flag
        char input[128];
        ...
        char *string[256];
        // store strings in here that needs to be executed
        char *line;
        FILE *batch;
        // file pointer
        ...
        //read in the commands
        {
            ...
            //seperate the commands using string token
            while (line != NULL)
            {
                string[i] = line;
                i++;
                line = strtok(NULL, delimit);
                //command that needs to be executed will be stored in here
            }
            
            for (j = 0; j < i; j++)
            {
                if((strstr(string[j],"quit")) != NULL || (strstr(string[j],"exit")) != NULL)
                { //"quit" and "exit" command to end program
                    exit_flag = TRUE;
                    //boolean flag
                }
    ...
    { // error checking for arguments that are greater than 3
